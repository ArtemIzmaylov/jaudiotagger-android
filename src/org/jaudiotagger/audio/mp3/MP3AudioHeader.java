...
public class MP3AudioHeader implements AudioHeader
{
    ...
    protected void setBitRate()
    {
        ...
        if (mp3XingFrame != null && mp3XingFrame.isVbr())
        {
            ...
            {
                bitrate = (long) (((long) mp3XingFrame.getAudioSize() * BITS_IN_BYTE_MULTIPLIER) / (timePerFrame * getNumberOfFrames() * Utils.KILOBYTE_MULTIPLIER));
            }
            else
            {
        ...
        else if (mp3VbriFrame != null)
        {
            ...
            {
                bitrate = (long) (((long) mp3VbriFrame.getAudioSize() * BITS_IN_BYTE_MULTIPLIER) / (timePerFrame * getNumberOfFrames() * Utils.KILOBYTE_MULTIPLIER));
            }
            else
            {
                bitrate = (long) (((fileSize - getMp3StartByte()) * BITS_IN_BYTE_MULTIPLIER) / (timePerFrame * getNumberOfFrames() *  Utils.KILOBYTE_MULTIPLIER));
